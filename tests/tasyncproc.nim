...
var shWithPrefix = ProcArgs(prefixCmd: @["sh", "-c"], options: {QuoteArgs,
        CaptureOutput, CaptureOutputErr})
var shUnquotedWithPrefix = ProcArgs(prefixCmd: @["sh", "-c"], options: {
        CaptureOutput, CaptureOutputErr})
...
proc main() {.async.} =
    ...
    test "workingDir":
        let workingDir = "/home"
        check (await sh.runGetOutput(@["pwd"], ProcArgsModifier(
                workingDir: some workingDir))) == workingDir
        check getFdCount() == 5
    ...
    test "With Tee output":
        ...
        check (await sh.runGetOutput(@["echo", "Hello"], ProcArgsModifier(
                output: some outStream.writer.AsyncIoBase))) == "Hello"
    ...
    test "environment":
        ...
        check (await sh.runGetOutput(@["env"], ProcArgsModifier(env: some {
                "VAR": "VALUE"}.toTable))) == "VAR=VALUE"
        putEnv("KEY", "VALUE")
    ...
    test "Stderr":
        ...
        check (await shUnquotedWithPrefix.merge(toRemove = {
                CaptureOutputErr}).run(@["echo Hello >&2"])).outputErr == ""
    ...
    test "EnvOnComdline: Quoted":
        ...
        check "VAR=VALUE" in (await shWithPrefix.runGetLines(@["env"],
                ProcArgsModifier(
            env: some {"VAR": "VALUE"}.toTable,
            toAdd: {SetEnvOnCmdLine}
        )))
        # Side effect, because quoted, space is well captured
        check "VAR=VALUE SPACED" in (await shWithPrefix.runGetLines(@["env"],
                ProcArgsModifier(
    ...
    test "EnvOnComdline: Unquoted":
        # sh add a few env variable
        check "VAR=VALUE" in (await shUnquotedWithPrefix.runGetLines(@["env"],
                ProcArgsModifier(
            env: some {"VAR": "VALUE"}.toTable,
            toAdd: {SetEnvOnCmdLine}
        )))
        # >>> @["sh", "-c", "export VAR=VALUE SPACED; env"]
        check "VAR=VALUE SPACED" notin (await shUnquotedWithPrefix.runGetLines(
                @["env"], ProcArgsModifier(
            env: some {"VAR": "VALUE SPACED"}.toTable,
            toAdd: {SetEnvOnCmdLine}
        )))
        check getFdCount() == 5
    ...
    test "Interactive with input capture":
        var sh2 = ProcArgs(prefixCmd: @["sh", "-c"], options: {Interactive,
