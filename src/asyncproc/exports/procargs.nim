...
type
    ProcArgs* = ref object
        ...
        #[
            ...
            logFn: this proc will be called only if appropriate option is set and after childproc have been awaited
...
proc buildCommand(procArgs: ProcArgs, postfixCmd: seq[string]): seq[string] =
    ...
    stringCmd.add (
        if SetEnvOnCmdLine in procArgs.options:
            (if UseParentEnv in procArgs.options:
                newParentEnv().mergeEnv(procArgs.env)
            else:
                procArgs.env
